...
from ita.men_parameter_levels
where parameter='year'
);

create table if not exists ita.men_factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
truncate table ita.men_factors;
...
parameter,
avg(raw_factor)
from ita.men_factors
where parameter not in ('o_div','d_div')
group by parameter
);

update ita.men_factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=men_factors.parameter;

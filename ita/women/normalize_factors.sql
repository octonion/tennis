...
from ita.women_parameter_levels
where parameter='year'
);

create table if not exists ita.women_factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
truncate table ita.women_factors;
...
parameter,
avg(raw_factor)
from ita.women_factors
where parameter not in ('o_div','d_div')
group by parameter
);

update ita.women_factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=women_factors.parameter;
